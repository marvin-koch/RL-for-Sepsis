...
class Critic_RNN(nn.Module):
    ...
    def get_hidden_states(self, prev_actions, rewards, observs, device="cpu"):
        ...
        inputs = torch.cat((input_a, input_r, input_s), dim=-1).to(device)
    ...
    def forward(self, prev_actions, rewards, observs, current_actions, device="cpu"):
        """
        ...
        self.current_shortcut_embedder = self.current_shortcut_embedder.to(device)
        ...
        if current_actions.shape[0] == observs.shape[0]:
            ...
            self.current_shortcut_embedder = self.current_shortcut_embedder.to(device)
            observs, current_actions = observs.to(device), current_actions.to(device)

...
class Actor_RNN(nn.Module):
    ...
    def get_hidden_states(
        self, prev_actions, rewards, observs, initial_internal_state=None, device="cpu"
    ):
        # all the input have the shape of (1 or T+1, B, *)
        # get embedding of initial transition
        
        rewards = rewards.to(device)
        observs = observs.to(device)
        ...
        input_a = self.action_embedder(prev_actions).to(device)
        input_r = self.reward_embedder(rewards).to(device)
        input_s = self._get_obs_embedding(observs).to(device)
        ...
        if initial_internal_state is None:  # initial_internal_state is zeros
            ...
            initial_internal_state = (initial_internal_state[0].to(device), initial_internal_state[1].to(device))
            output, current_internal_state = self.rnn(inputs, initial_internal_state)
            return output, current_internal_state

    def forward(self, prev_actions, rewards, observs, device="cpu"):
        """
        ...
        hidden_states = self.get_hidden_states(
            prev_actions=prev_actions, rewards=rewards, observs=observs, device=device
        )

        # 2. another branch for current obs
        self.current_observ_embedder.to(device)
        observs = observs.to(device)
        ...
        return self.algo.forward_actor(actor=self.policy, observ=joint_embeds, device=device)

...
class SinePositionalEncoding(nn.Module):
    def __init__(self, max_len, hidden_size, device) -> None:
        super().__init__()
        self.device = device
        ...
        self.pe = ptu.from_numpy(pe).to(self.device)  # (max_len, d)
...
class LearnedPositionalEncoding(nn.Module):
    def __init__(self, max_len, hidden_size, device) -> None:
        super().__init__()
        self.device = device
        self.pe = nn.Embedding(max_len, hidden_size).to(self.device)
...
class GPT2(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.embed_timestep = Encoding(max_seq_length + 2, hidden_size, device).to(device)
    ...
    def forward(self, input_embeds, h_0):
        ...
        if h_0 is None:  # training: entire sequence as input
            ...
            timesteps = torch.arange(0, length)
            pkv = None
            ...
            if history_length > self.max_history_length:  # confirmed this is correct
                ...
                ).to(self.device)  # match the training
    ...
    def _forward(self, input_embeds, timesteps, pkv):
        ...
        ).to(self.device)  # (T, 1, hidden_size)
    ...
    def get_zero_internal_state(self, batch_size=None):
        if batch_size is None:  # inference, batch_size=1
            pkv = None
            initial_timestep = torch.arange(0, 1).to(self.device)
            return (pkv, initial_timestep, ptu.zeros((1, 1, self.hidden_size)).float().to(self.device))

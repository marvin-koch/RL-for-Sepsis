...
class Experiment(object): 
    def __init__(self, domain, train_data_file, validation_data_file, test_data_file, writer, minibatch_size, device,
        ...
        self.train_loader = DataLoader(self.train_dataset, batch_size=self.minibatch_size, shuffle=False)
        ...
        with torch.no_grad():
            for i_set, loader in enumerate([self.train_loader, self.val_loader, self.test_loader]):
                ...
                for dem, ob, acc, l, t, scores, rewards, outcomes, idx in loader:
                    ...
                    acc = acc[:, :max_length, :].to(self.device)
                    scores = scores[:, :max_length, :].to(self.device)
                    rewards = rewards[:, :max_length].to(self.device)
                    outcomes = outcomes[:, :max_length].to(self.device)

                    cur_obs, next_obs = ob[:, :-1, :].to(self.device), ob[:, 1:, :].to(self.device)
                    cur_dem, next_dem = dem[:, :-1, :].to(self.device), dem[:, 1:, :].to(self.device)
                    cur_scores = scores[:, :-1, :].to(self.device)
                    next_scores = scores[:, 1:, :].to(self.device)

                    cur_actions_c = acc[:, :-1, :].to(self.device)
                    next_actions_c = acc[:, 1:, :].to(self.device)
                    cur_rewards = rewards[:, :-1].to(self.device)
                    outcomes = outcomes[:, :-1].to(self.device)


                    mask = (cur_obs==0).all(dim=2).to(self.device)
                    ...
                    if self.autoencoder != 'None':
                        ...
                        cur_rep = cur_rep[~mask]
                        next_rep = next_rep[~mask]

                        cur_actions_c = cur_actions_c[~mask]
                        next_actions_c = next_actions_c[~mask]
                        cur_scores = cur_scores[~mask]
                        next_scores = next_scores[~mask]
                        cur_rewards = cur_rewards[~mask]
    ...
    def train_TD3_policy(self, actor_learning_rate, critic_learning_rate, model, task, load, loss, reward, reward_param):
...
#
        ...
        if model == 'lstm':
            td3_utils.run_LSTM_TD3(replay_buffer, test_replay_buffer, state_dim, self.action_dim, self.device, params, self.writer, task, load, loss, reward, reward_param)
        else:
            td3_utils.run_Transformer_TD3(replay_buffer, test_replay_buffer, state_dim, self.action_dim, self.device, params, self.writer, task, load, loss, reward, reward_param)
